# Firmware & MCU integration (RGBDesk nodes)

Moved out of CLAUDE.md so it is loaded only when working on firmware.
The PlatformIO project lives at `firmware/esp32-ble-scanner/`; the tree
described below (root `platformio.ini`, `src/`, `lib/`, `include/`) is the
target layout as nodes are built out, not the current state.
